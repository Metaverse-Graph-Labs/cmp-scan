...
describe("Contract APIs", function () {
  ...
  it("Verify & Publish Contract Source Code", async () => {
    await setTimeout(WAITING_TIME);
    try {
      const res = await axios.post(
        `${BASE_URI}module=contract&action=verify&apikey=${API_KEY}`,
        {
          contract_address: "0x944ea57e97886fe8881a1fe5aabcd0d889490c97",
...
contract GreenToken is ERC20, ERC20Burnable, Ownable {
  ...
  it.only("Deploy, Verify & Publish Contract Source Code", async () => {
    //   GreenToken;
    const now = new Date();
    const _timestamp = now.getTime();
    const GreenToken = await ethers.getContractFactory("GreenToken20");
    const greenToken = await GreenToken.deploy(
      "0x89d903afFba1e0A9Aab4D70B260Ae2b220c53eD2",
      _timestamp
    );

    const tx = await greenToken.deployed();
    // return;

    await setTimeout(WAITING_TIME * 12 * 2);
    console.log("Deployed: ", greenToken.address);
    // await network.provider.send("evm_mine");
    try {
      const _abiArgs = `00000000000000000000000089d903affba1e0a9aab4d70b260ae2b220c53ed200000000000000000000000000000000000000000000000000000${_timestamp.toString(
        16
      )}`;
      const res = await axios.post(
        `${BASE_URI}module=contract&action=verify&apikey=${API_KEY}`,
        {
          contract_address: greenToken.address.toString(),
          //   contract_address: "0x92a471c27D56b34Aaf8C64F67a11E15FEA1c50fA",
          compiler_type: "SingleFile",
          compiler_version: "v0.8.9+commit.e5eed63a",
          license_type: "MIT",
          Continue: false,
...
contract GreenToken is ERC20, ERC20Burnable {
    
    address public owner;
    uint256 public timestamp;

    constructor(address _owner, uint256 _timestamp) ERC20("Green Token", "GREEN") {
        owner = _owner;
